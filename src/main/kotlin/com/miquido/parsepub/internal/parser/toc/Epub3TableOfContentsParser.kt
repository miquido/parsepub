...
internal class Epub3TableOfContentsParser : TableOfContentsParser {
    ...
    override fun parse(
        ...
        tocNav.getFirstElementByTag(OL_TAG)
            .orValidationError { attributeLogger?.logMissingAttribute(OL_TAG, TABLE_OF_CONTENTS_TAG) }
            ?.childNodes.forEach {
            if (it.isNavPoint()) {
    ...
    override fun createNavigationItemModel(it: Node): NavigationItemModel {
        ...
        val label = ref?.textContent
            .orValidationError { validationAttr?.logMissingAttribute(LABEL_FIELD_NAME, TABLE_OF_CONTENTS_TAG) }
        val source = ref?.getAttribute(HREF_ATTR)
            .orValidationError { validationAttr?.logMissingAttribute(HREF_ATTR, TABLE_OF_CONTENTS_TAG) }
